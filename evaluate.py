...
class CloudQuestionProvider:
    ...
    def generate_many(self, node: Any, count: int) -> list[dict[str, str]]:
        """Generate up to ``count`` diverse question/answer pairs from one chunk.

        Each call targets a different angle and lists the questions already produced, so a
        small single-chunk source (e.g. a one-file model card) still yields a usable
        evaluation set instead of a single question.
        """
        if not self.enabled:
            raise RuntimeError("Cloud RAGAS disabled; set USE_CLOUD_FREE_TIER_RAGAS=1 and ALLOW_CLOUD_FREE_TIER=1.")
        ...
        context = _node_text(node)[:6000]
        angles = (
            "its core purpose or what it does",
            "a specific technical detail, name, or number",
            "its inputs, data, or dependencies",
            "its results, metrics, or outcomes",
            "its configuration, setup, or usage",
        )
        items: list[dict[str, str]] = []
        for index in range(max(int(count), 1)):
            asked = "; ".join(item["question"] for item in items)
            prompt = (
                "Generate one evaluation question and one ground-truth answer about "
                f"{angles[index % len(angles)]} from this context. The answer must be fully "
                "supported by the context. "
                + (f"Make it different from these earlier questions: {asked}. " if asked else "")
                + "Return strict JSON with keys question and ground_truth.\n\n"
                f"Context:\n{context}"
            )
            try:
            ...
            if question and ground_truth:
                items.append({"question": question, "ground_truth": ground_truth})
        return items
...
def generate_golden_dataset(
    ...
    llm_enabled = cloud_refinement_provider is not None

    if llm_enabled:
        # LLM-first path: every question is generated by the cloud provider from a sampled
        # chunk, so the dataset is specific to the ingested source and varies between runs
        # instead of replaying a fixed, domain-templated prefix. Several questions are drawn
        # per chunk (scaled to the target size) so small single-file sources still produce a
        # usable evaluation set. No deterministic seeding or domain hardcoding is applied here.
        selected_nodes = list(question_nodes or nodes)
        per_node = min(5, max(1, -(-final_limit // max(len(selected_nodes), 1))))
        candidates: list[dict[str, str]] = []
        for node in selected_nodes:
            try:
                generated = cloud_refinement_provider.generate_many(node, per_node)
            except Exception:
                generated = []
            for gen in generated:
                item = _validated_golden_item(
                    gen.get("question", ""),
                    gen.get("ground_truth", ""),
                    _node_text(node),
                    _source_doc(node),
                )
                if item:
        ...
        if not candidates:
            # Cloud produced nothing usable; fall back to one item per node via the chain.
            for node in selected_nodes:
                try:
                    candidates.append(generate_golden_item(node, providers))
                except RuntimeError:
                    continue
    else:
        # Offline fallback: no LLM available, so fall back to the structured (deterministic)
        # extractive heuristics. Reproducible by design.
    ...
    if llm_enabled:
        # No privileged "realistic" prefix in LLM mode: rank all generated items together.
        realistic: list[dict[str, str]] = []
        remaining = filter_best_golden_items(candidates, limit=final_limit)
    else:
...
def run_evaluation(
    ...
    if use_real_ragas and cloud_client is not None:
        for index, strategy in enumerate(STRATEGIES):
            ...
            if real_scores:
                # Keep the offline lexical metrics alongside the cloud RAGAS scores. Free-tier
                # models frequently return a non-finite faithfulness, which gets dropped from
                # real_scores; without the lexical proxy the metric would vanish entirely from
                # the chart. Merging keeps a labeled fallback while the degraded status below
                # still flags that the cloud faithfulness itself was missing.
                summaries[strategy] = {**summaries[strategy], **real_scores}
