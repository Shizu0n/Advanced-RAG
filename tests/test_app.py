...
class AppHelperTests(unittest.TestCase):
    ...
    def test_clear_source_cache_removes_chroma_dir_when_collection_delete_finds_invalid_db(self):
        with TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            raw_dir = root / "data" / "raw"
            chroma_dir = root / "chroma_db"
            ...
            (chroma_dir / "chroma.sqlite3").write_text("stale", encoding="utf-8")
            mock_client = MagicMock()
            mock_client.delete_collection.side_effect = RuntimeError("no such table: tenants")
            mock_chromadb = MagicMock()
            mock_chromadb.PersistentClient.return_value = mock_client
